...
public class BahmniDiagnosisController extends BaseRestController {
    ...
    @ResponseBody
    public List<BahmniDiagnosisRequest> search(@RequestParam("patientUuid") String patientUuid,
                                               @RequestParam(value = "fromDate", required = false) String date,
                                               @RequestParam(value = "visitUuid", required = false) String visitUuid) throws Exception {
        if (visitUuid != null) {
